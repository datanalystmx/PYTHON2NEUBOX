...
# 2. team_stats_all.json — {team_name_lower: {total: {}, home: {}, road: {}}}
team_stats_all = {}
...
# 3. opp_stats_all.json — {team_name_lower: {total: {}, home: {}, road: {}}}
opp_stats_all = {}
...
# 4. advanced_stats.json — {team_name_lower: {total: {}, home: {}, road: {}}}
adv_stats_all = {}
...
for sheet in quarter_sheets:
    if sheet in all_sheets:
        quarter_stats[sheet] = {
            str(row['TEAM_NAME']).lower().strip(): row
            for row in all_sheets[sheet].to_dict(orient='records')
            if row.get('TEAM_NAME')
        }
guardar_json('quarter_stats', quarter_stats)

# 6. player_stats.json — {sheet_name: [lista de jugadores]}
player_sheets = [
...
# 7. standings.json — lista plana con TeamCity+TeamName como clave lower
if 'Standings' in all_sheets:
    standings_list = all_sheets['Standings'].to_dict(orient='records')
    guardar_json('standings', standings_list)
