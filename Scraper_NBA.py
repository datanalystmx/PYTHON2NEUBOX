...
# 2. team_stats_all.json — {team_full_name_lower: {total: {}, home: {}, road: {}}}
# Clave = "oklahoma city thunder" (TeamCity + TeamName), igual que standings y schedule
team_stats_all = {}
for key, sheet in [('total','Total_Stats'), ('home','Home_Stats'), ('road','Road_Stats')]:
    if sheet in all_sheets:
        for row in all_sheets[sheet].to_dict(orient='records'):
            name = (str(row.get('TEAM_CITY', '')) + ' ' + str(row.get('TEAM_NAME', ''))).lower().strip()
            if not name or name == ' ':
                # fallback a TEAM_NAME si no hay TEAM_CITY
                name = str(row.get('TEAM_NAME', '')).lower().strip()
            if name:
                if name not in team_stats_all:
                    team_stats_all[name] = {}
                team_stats_all[name][key] = row
guardar_json('team_stats_all', team_stats_all)

# 3. opp_stats_all.json — {team_full_name_lower: {total: {}, home: {}, road: {}}}
opp_stats_all = {}
    ...
    if sheet in all_sheets:
        for row in all_sheets[sheet].to_dict(orient='records'):
            ...
            if not name or name == ' ':
                name = str(row.get('TEAM_NAME', '')).lower().strip()
            if name:
                if name not in opp_stats_all:
                    opp_stats_all[name] = {}
                opp_stats_all[name][key] = row
guardar_json('opp_stats_all', opp_stats_all)

# 4. advanced_stats.json — {team_full_name_lower: {total: {}, home: {}, road: {}}}
adv_stats_all = {}
    ...
    if sheet in all_sheets:
        for row in all_sheets[sheet].to_dict(orient='records'):
            ...
            if not name or name == ' ':
                name = str(row.get('TEAM_NAME', '')).lower().strip()
            if name:
                if name not in adv_stats_all:
                    adv_stats_all[name] = {}
                adv_stats_all[name][key] = row
guardar_json('advanced_stats', adv_stats_all)

# 5. quarter_stats.json — {sheet_name: {team_name_lower: {}}}
# 5. quarter_stats.json — estructura anidada que espera el PHP:
# quarterStats['trad']['q1']['total'][team_lower]
# quarterStats['opp']['q1']['road'][team_lower]
quarter_map = {
...
for sheet, (qtype, period, loc) in quarter_map.items():
    if sheet in all_sheets:
        if qtype not in quarter_stats:
            quarter_stats[qtype] = {}
        if period not in quarter_stats[qtype]:
            quarter_stats[qtype][period] = {}
        quarter_stats[qtype][period][loc] = {
            (str(row.get('TEAM_CITY', '')) + ' ' + str(row.get('TEAM_NAME', ''))).lower().strip(): row
            for row in all_sheets[sheet].to_dict(orient='records')
            if row.get('TEAM_NAME')
        }
guardar_json('quarter_stats', quarter_stats)

# 6. player_stats.json — {player_id: {total:{}, home:{}, road:{}, name, team, position, age}}
# PHP accede: playerStats[key]['total'], p['name'], p['team'], p['position'], p['age']
player_stats = {}
sheet_to_cond = {
    'Total_Player_Stats': 'total',
    'Players_Home':       'home',
    'Players_Road':       'road',
}
# Construir mapa de posiciones desde Player_Positions
pos_map = {}
...
for sheet, cond in sheet_to_cond.items():
    ...
    for row in all_sheets[sheet].to_dict(orient='records'):
        ...
        if pid not in player_stats:
            player_stats[pid] = {
                ...
                'position': pos_map.get(row.get('PLAYER_ID'), {}).get('position', ''),
                'age':      pos_map.get(row.get('PLAYER_ID'), {}).get('age', ''),
            }
        player_stats[pid][cond] = row
guardar_json('player_stats', player_stats)

# 7. standings.json — {team_full_name_lower: {campos normalizados}}
if 'Standings' in all_sheets:
    standings_dict = {}
    for row in all_sheets['Standings'].to_dict(orient='records'):
        team_name = (str(row.get('TeamCity', '')) + ' ' + str(row.get('TeamName', ''))).strip().lower()
        if not team_name:
            continue
